...
function atlasWebGLSupported(){try{var c=document.createElement('canvas');return !!(c.getContext('webgl2')||c.getContext('webgl')||c.getContext('experimental-webgl'))}catch(e){return false}}
...
function initMap(id){atlasTrace('map.init.enter',{id:id,maplibre:!!window.maplibregl,deck:!!window.deck});
  ...
  if(initialStatus)initialStatus.textContent='Inicializando MapLibre · '+(window.maplibregl?'Motor disponible':'Esperando el motor')+
    ' · WebGL: '+(atlasWebGLSupported()?'Sí':'No');
  ...
  var supported=atlasWebGLSupported();
  atlasTrace('map.webgl.supported',{supported:supported,mapCtor:!!(window.maplibregl&&window.maplibregl.Map),keys:window.maplibregl?Object.keys(window.maplibregl).slice(0,30):[]});
  if(!supported||!window.maplibregl||typeof window.maplibregl.Map!=='function'){atlasStatus('Sin WebGL · mostrando mapa de España local',true);var status=document.querySelector('.map-diagnostics-status');if(status)status.textContent='WebGL deshabilitado en el navegador';return}
  try{
    el.innerHTML='';
