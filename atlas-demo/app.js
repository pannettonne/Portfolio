...
function initMap(id){
  ...
  if(!maplibregl.supported()){fallback(el);atlasStatus('WebGL no disponible en este dispositivo',true);return}
  try{
    el.innerHTML='';
    ...
    map.on('error',function(e){
      if(map!==state.map)return;
      var error=e&&e.error;console.warn('ATLAS map resource error:',error||e);atlasLastMapError=String(error&&error.message||'Fallo de carga del recurso cartográfico');
      tileFailures++;
